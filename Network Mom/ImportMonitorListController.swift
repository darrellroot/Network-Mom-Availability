    ...
    @IBAction func importButton(_ sender: NSButton) {
        ...
        for line in lines {
            ...
            if let ipv4string = candiateIP.ipv4address {
                ...
                if let newMonitor = MonitorIPv4(ipv4string: ipv4string, hostname: hostname, comment: comment) {
            ...
            if let ipv6 = IPv6Address(candiateIP) {
                ...
                if let newMonitor = MonitorIPv6(ipv6: ipv6, hostname: hostname, comment: comment) {
                    newMonitor.comment = comment
